...
from video_studio import API_VERSION, __version__, _env
...
def test_the_flag_reader_really_reads_the_parser():
    ...
    flags = _init_flags()
    assert {"--station", "--mode", "--yes", "--non-interactive", "--migrate", "--undo",
            "--dry-run"} <= set(flags)
...
def _names_code_reads():
    out = set()
    for p in (ROOT / "video_studio").rglob("*.py"):
...
def test_env_example_marks_what_a_dotenv_can_never_reach():
    """Biến chỉ `os.environ` tới được (mã đọc thẳng, hoặc tên trỏ chính trạm này) phải được
    đánh dấu rõ. Không có luật này thì người dùng điền xong vẫn không có gì đổi — kiểu hỏng
    khó chịu nhất vì nó im lặng."""
    text = ENV_EXAMPLE.read_text(encoding="utf-8")
    assert "[MÔI TRƯỜNG THẬT]" in text
    for name in _env._NEVER_FROM_DOTENV:
        assert name in _example_names(), f"{name} phải có trong khuôn, kèm lý do không đọc được"
